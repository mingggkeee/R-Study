var1 <- c(10, 20, 30, 40, 50) # y축 설정 - x축은 자동으로 따라온다.
plot(var1)
...
#
prev_par <- par(no.readonly = TRUE) # 기존 그래픽옵션 읽어서 저장하기 

par(mfrow = c(1, 3)) # 그래프 여러개 그리기기
plot(v1, type = 'o')
plot(v1, type = 's')
plot(v1, type = 'l')
...
name <- c('장동건 대표이사', '일지매 부장', '김유신 과장', '손흥민 대리',
          ...
          '광개토 과장', '정몽주 대리')
pemp <- c('장동건 대표이사', '장동건 대표이사', '일지매 부장', '김유신
과장', '김유신 과장', '장동건 대표이사', '이순신 부장', '유관순 과장',
...
stars(total, flip.labels = FALSE, draw.segments = FALSE,
      frame.plot = TRUE, full =TRUE,
      main = '학생별 과목별 성적분석 - STAR Chart')

stars(total, flip.labels = F, draw.segments = T, frame.plot = T,
...
#
score_kem <- read.csv(
  "data-files\\학생별과목별성적-국영수-new.csv", header = T)
...
graph3 <- graph1 + geom_point(aes(color = factor(am)))
graph3
...
graph6 <- graph1 + geom_point(aes(shape = factor(am),
                                  size = wt))
graph6

graph7 <- graph1 + geom_point(aes(shape = factor(am),
                                  color = factor(am),
                                  size = wt)) +
...
ggplot(sort_score, aes(x = 과목, y = 점수, color = 이름,
                       group = 이름, fill = 이름)) + geom_line() + geom_point(size = 6, shape = 22)
